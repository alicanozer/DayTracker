...
struct ContentView: View {
    ...
    static let serifFont = Font.custom(serifFontName, size: 17)
            ...
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("\(dateFormatter.string(from: range.startDate)) - \(dateFormatter.string(from: range.endDate))")
                        .font(.custom(Self.serifFontName, size: 17).weight(.semibold))
                ...
                Text(range.isInclusive ? "Inclusive" : "Exclusive")
                    .font(.custom(Self.serifFontName, size: 14).weight(.semibold))
    ...
    private var summaryAndAddButtonSection: some View {
        VStack(spacing: 10) {
            HStack(spacing: 8) {
                Label { Text("\(dateRangeManager.totalIncludedDays)").bold().foregroundColor(Self.textPrimary).font(.custom(Self.serifFontName, size: 17)) } icon: { Text("Included").foregroundColor(Self.textSecondary).font(.custom(Self.serifFontName, size: 14)) }
                    .labelStyle(VerticalLabelStyle())
                Divider().frame(height: 24)
                Label { Text("\(dateRangeManager.totalExcludedDays)").bold().foregroundColor(Self.textPrimary).font(.custom(Self.serifFontName, size: 17)) } icon: { Text("Excluded").foregroundColor(Self.textSecondary).font(.custom(Self.serifFontName, size: 14)) }
                    .labelStyle(VerticalLabelStyle())
                Divider().frame(height: 24)
                Label { Text("\(dateRangeManager.totalDays)").bold().foregroundColor(Self.textPrimary).font(.custom(Self.serifFontName, size: 17)) } icon: { Text("Final").foregroundColor(Self.textSecondary).font(.custom(Self.serifFontName, size: 14)) }
                    .labelStyle(VerticalLabelStyle())
            }
        ...
        VStack(spacing: 16) {
            ...
            VStack(spacing: 8) {
                DatePicker("Start Date", selection: $startDate, displayedComponents: .date)
                    ...
                    .cornerRadius(8)
                    .foregroundColor(Self.textPrimary)
                    .font(.custom(Self.serifFontName, size: 17))
                ...
                if endDate <= startDate {
                    Text("End date must be after start date.")
                        ...
                        .font(.custom(Self.serifFontName, size: 14))
                }
                Button(action: {
                    addDateRange()
                    showAddSheet = false
